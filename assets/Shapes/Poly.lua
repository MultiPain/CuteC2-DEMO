...
function Poly:onPropertiesDraw(ui)
	...
	if (ui:treeNode("Vertices")) then 
		local count = self.collisionShape:getVertexCount()
		local deleteIndex = 0
		...
		for i = 1, count do 
			...
			local vx, vy = self.collisionShape:getVertex(i, true)
			local newX, newY, moved = ui:dragFloat2("#"..i, vx, vy)
			
			if (moved) then 
				local deltaX = vx - newX
				local deltaY = vy - newY
				self:onVertexMove(i, deltaX, deltaY)
			end
		end
		
		if (deleteIndex > 0) then 
			self.collisionShape:removeVertex(deleteIndex)
		end
